...
<head>
  ...
  <title><?php echo Plug_Lang('软件销售'); ?></title>
  <style>
...
<body class="w<?php echo $wrap_mode; ?>">
  <div class="wrap">
    <div class="hd">
      <h1><?php echo Plug_Lang('软件销售'); ?></h1>
    </div>
    <div class="app-list">
      <?php if (empty($app_list)) { ?>
        <p style="color:#999;padding:40px 0;text-align:center"><?php echo Plug_Lang('暂无销售软件'); ?></p>
      <?php } else {
        foreach ($app_list as $a) {
          $detail_url = 'index.php?m=webapi&c=appsale&a=detail&daihao=' . (int)$a['app_daihao'];
          if ($u !== '') {
