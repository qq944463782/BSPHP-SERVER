...
<body class="w<?php echo $wrap_mode; ?>">
  <div class="wrap">
    <?php if ($show_header) { ?><div class="hd"><h1><?php echo Plug_Lang('续费充值'); ?></h1></div><?php } ?>
    <form id="form1" name="form1" method="post" action="">
      <div class="bd">
        <div class="fg">
          <label><?php echo Plug_Lang('充值账号'); ?></label>
          <input name="user_user" value="<?php echo htmlspecialchars($user_user ?? ''); ?>" type="text" placeholder="<?php echo Plug_Lang('充值账号'); ?>">
        </div>
        <div class="fg">
          <label><?php echo Plug_Lang('充值卡号'); ?></label>
          <input name="ka_name" value="<?php echo htmlspecialchars($ka_name ?? ''); ?>" type="text" placeholder="<?php echo Plug_Lang('充值卡卡号'); ?>">
        </div>
        <div class="fg">
          <label><?php echo Plug_Lang('充值卡密码'); ?></label>
          <input name="ka_pwd" value="<?php echo htmlspecialchars($ka_pwd ?? ''); ?>" type="text" placeholder="充值卡密码,没有可空">
        </div>
        <?php if (!empty($log_name)) { ?><div class="msg"><?php echo htmlspecialchars($log_name); ?></div><?php } ?>
        <button type="submit" name="Submitadd" value="1" class="btn"><?php echo Plug_Lang('确认充值'); ?></button>
      </div>
    </form>
  </div>
</body>
