...
<body class="w<?php echo $wrap_mode; ?>">
  <div class="wrap">
    <?php if ($show_header) { ?><div class="hd"><h1><?php echo Plug_Lang('账号在线信息'); ?></h1></div><?php } ?>
    <form id="form1" name="form1" method="post" action="">
      <div class="bd">
        <table>
          ...
          <tbody>
            <?php $i=1; while ($value = Plug_Pdo_Fetch_Assoc($db_array_value)) { ?>
            <tr>
              ...
              <td><span class="tag tag-time"><?php echo date('Y-m-d H:i:s', $value['links_add_time']); ?></span></td>
              <td><span class="tag tag-time"><?php echo date('Y-m-d H:i:s', $value['links_out_time']); ?></span></td>
              <td><a class="lnk" href="index.php?m=webapi&c=links_login&a=index&daihao=<?php echo $daihao; ?>&id=<?php echo $value['links_id']; ?>">T</a></td>
            </tr>
            <?php $i++; } ?>
          </tbody>
        </table>
