...
<head>
  ...
  <title><?= htmlspecialchars($appinfo['app_sale_title'] ?? $appinfo['app_name'] ?? '购买现卡') ?></title>
  ...
  <style>
    ...
    .package-stock { font-size: 12px; color: #0d9488; margin-top: 4px; }
    .package-stock.low { color: #b45309; }
    .package-stock.zero { color: #dc2626; }
    ...
    .price-summary .stock-warn { color: #dc2626; font-size: 13px; margin-top: 4px; }
...
<body class="w<?= $wrap_mode ?>">
  <div class="container">
    ...
    <?php } elseif ($appinfo) { ?>
      <form id="gencardForm" method="get" action="index.php?m=webapi&c=salecard_salecard&a=do_pay" target="_blank">
        <input type="hidden" name="daihao" value="<?= (int)($daihao ?? 0) ?>">
        <input type="hidden" name="m" value="webapi">
        <input type="hidden" name="c" value="salecard_salecard">
        ...
        <div class="card brand">
          ...
          <div>
            ...
            <div class="brand-desc"><?= htmlspecialchars($appinfo['app_sale_desc'] ?? '销售导入的现卡 · 支付成功后凭订单号查询') ?></div>
        ...
        <div class="card">
          ...
          <div class="grid">
            ...
            foreach ($card_list as $c) {
              $stock = isset($c['stock_count']) ? (int)$c['stock_count'] : 0;
              $stockClass = $stock <= 0 ? 'zero' : ($stock <= 5 ? 'low' : '');
            ?>
              <label class="package <?= $stock <= 0 ? 'no-stock' : '' ?>" data-jiage="<?= (float)$c['lei_jiage'] ?>" data-name="<?= htmlspecialchars($c['lei_name']) ?>" data-stock="<?= $stock ?>">
                <input type="radio" name="carid" value="<?= (int)$c['lei_id'] ?>" <?= $stock <= 0 ? 'disabled' : '' ?>>
                ...
                <div class="package-body">
                  ...
                  <div class="package-desc"><?= htmlspecialchars(isset($c['lei_desc']) ? $c['lei_desc'] : '现卡销售，支付后从库存分配') ?></div>
                  <div class="package-stock <?= $stockClass ?>">库存 <?= $stock ?> 张</div>
                </div>
              </label>
            <?php } ?>
          </div>
          <?php if (empty($card_list)) { ?>
            <p style="color:#64748b;margin:12px 0 0;font-size:14px;">暂无可用卡类，或该卡类下无「用销售现卡」库存。请在后台制卡导入时选择「用销售现卡」。</p>
          <?php } ?>
        </div>
        ...
        <div class="card">
          ...
          <p style="color:#64748b;margin:8px 0 0;font-size:13px;">数量不可超过当前卡类现卡库存，支付后从库存分配卡密。</p>
        </div>
        ...
        <?php if (!empty($card_list) && !empty($pay_channels)) { ?>
          <div style="margin-top:10px;">
            <button type="button" class="btn" id="gencardSubmitBtn">立即购买现卡</button>
          </div>
        <?php } ?>
      </form>
      ...
      <div class="card" style="margin-top:18px;">
        ...
        <div style="display:flex;gap:10px;">
          <input type="text" placeholder="请输入SALEIMP开头订单号查询" id="orderInput" class="input">
  ...
  <script>
    ...
    $(function() {
      ...
      $('input[name="carid"]:not(:disabled)').first().prop('checked', true);
      ...
      function updatePriceSummary() {
        ...
        if (qty < 1) qty = 1;
        if (qty > 99) qty = 99;
        var stock = $sel.length ? parseInt($sel.data('stock'), 10) || 0 : 0;
        if (qty > stock && stock >= 0) qty = Math.max(1, stock);
        $num.val(qty);
        var $box = $('#gencardPriceInfo');
        if (!$sel.length) {
          $box.text('请先选择卡类').find('.stock-warn').remove();
          return;
        }
        ...
        var html = '<span class="unit">' + namePart + '单价 ¥' + jiage.toFixed(2) + ' × ' + qty + ' 张</span> = <span class="total">合计 ¥' + total + '</span>';
        if (stock >= 0 && qty > stock) {
          html += '<div class="stock-warn">当前库存 ' + stock + ' 张，已自动调整为最多 ' + stock + ' 张</div>';
        }
        $box.html(html);
      }
      ...
      $('#gencardSubmitBtn').on('click', function() {
        ...
        if (shu < 1) shu = 1;
        if (shu > 99) shu = 99;
        var stock = $car.length ? parseInt($car.closest('.package').data('stock'), 10) || 0 : 0;
        if (stock >= 0 && shu > stock) {
          showToast('购买数量不能超过库存（' + stock + ' 张）', 'error');
          return;
        }
        ...
        $.ajax({
          url: 'index.php?m=webapi&c=salecard_salecard&a=do_pay',
          type: 'POST',
          dataType: 'json',
      ...
      $('#orderBtn').on('click', function() {
        ...
        var $box = $('#orderResult');
        if (!v) {
          $box.removeClass('success pending').addClass('order-result show error').text('请输入SALEIMP开头订单号');
          return;
        }
        $box.removeClass('success error').addClass('order-result show pending').text('查询中...');
        $.ajax({
          url: 'index.php?m=webapi&c=salecard_salecard&a=order_status',
          type: 'POST',
          dataType: 'json',
          ...
          if (d.status === 'paid') {
            ...
            if (d.has_card && d.cards && d.cards.length) {
              ...
              html += '<a href="index.php?m=webapi&c=salecard_salecard&a=show_card&order=' + encodeURIComponent(d.pay_id) + '" target="_blank" style="color:#2563eb;">单独打开查看卡密</a>';
              ...
              html += '<a href="index.php?m=webapi&c=salecard_salecard&a=show_card&order=' + encodeURIComponent(d.pay_id) + '" target="_blank" style="color:#2563eb;">单独打开查看卡密</a></div>';
            }
