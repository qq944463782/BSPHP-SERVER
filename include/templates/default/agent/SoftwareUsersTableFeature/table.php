...
<script>layui.config({ base: '<?php echo Plug_Get_Url_Statics() ?>style/' }).extend({ index: 'lib/index' }).use(['jquery', 'index', 'table'], function() {var jq=layui.jquery, table=layui.table;function searchWhere() {return {soso_id: document.getElementById('soso_id').value,soso: document.getElementById('soso').value,DESC: document.getElementById('DESC').value};}table.render({elem: '#test-table-toolbar',url: 'index.php?m=<?php echo Plug_Set_Get('m'); ?>&c=<?php echo Plug_Set_Get('c'); ?>&a=<?php echo Plug_Set_Get('a'); ?>_json&t=<?php echo Plug_Set_Get('t'); ?>',title: '<?php echo Plug_Lang('间接软件用户列表'); ?>',cellMinWidth: 110,height: 'full-250',where: searchWhere(),cols: [[{field: 'id', width: 90, title: 'ID'},{field: 'uid', minWidth: 150, title: '<?php echo Plug_Lang('用户UID'); ?>'},{field: 'user', minWidth: 150, title: '<?php echo Plug_Lang('用户名'); ?>'},{field: 'daihao', width: 110, title: '<?php echo Plug_Lang('软件代号'); ?>'},{field: 'agent_uid', width: 130, title: '<?php echo Plug_Lang('所属代理UID'); ?>'},{field: 'ip', minWidth: 130, title: 'IP'},{field: 'addtime', minWidth: 170, title: '<?php echo Plug_Lang('记录时间'); ?>'}]],page: true});jq('.layuiadmin-btn-useradmin').on('click', function() {table.reload('test-table-toolbar', {page: {curr: 1}, where: searchWhere()});});});</script>
</body>
</html>
