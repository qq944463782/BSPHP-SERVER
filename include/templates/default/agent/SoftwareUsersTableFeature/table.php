...
.use(['jquery', 'index', 'table'], function() {
  ...
  table.render({
    ...
    title: '<?php echo Plug_Lang('间接软件用户列表'); ?>',
    cellMinWidth: 110,
    height: 'full-250',
    where: searchWhere(),
    cols: [[
      {field: 'id', width: 90, title: 'ID'},
      {field: 'uid', minWidth: 150, title: '<?php echo Plug_Lang('用户UID'); ?>'},
      {field: 'user', minWidth: 150, title: '<?php echo Plug_Lang('用户名'); ?>'},
      {field: 'daihao', width: 110, title: '<?php echo Plug_Lang('软件代号'); ?>'},
      {field: 'agent_uid', width: 130, title: '<?php echo Plug_Lang('所属代理UID'); ?>'},
      {field: 'ip', minWidth: 130, title: 'IP'},
      {field: 'addtime', minWidth: 170, title: '<?php echo Plug_Lang('记录时间'); ?>'}
    ]],
    page: true
  });
