...
<script>layui.config({ base: '<?php echo Plug_Get_Url_Statics() ?>style/' }).extend({ index: 'lib/index' }).use(['jquery', 'index', 'table'], function() {var jq=layui.jquery, table=layui.table;var tableIns=table.render({elem: '#moneylog-table',id: 'moneylog-table',url: 'index.php?m=agent&c=CommissionLogsFeature&a=table_json',title: '<?php echo Plug_Lang('分佣金收入记录'); ?>',cellMinWidth: 110,height: 'full-260',where: { source: 'direct', soso_id: 2, DESC: 0, soso: '' },cols: [[{field: 'id', width: 80, title: '<?php echo Plug_Lang('编号'); ?>', sort: true},{field: 'log_amount', width: 110, title: '<?php echo Plug_Lang('分佣金额'); ?>', sort: true},{field: 'log_level', width: 80, title: '<?php echo Plug_Lang('级别'); ?>'},{field: 'log_order', minWidth: 260, title: '<?php echo Plug_Lang('订单号'); ?>'},{field: 'log_desc', minWidth: 220, title: '<?php echo Plug_Lang('描述'); ?>'},{field: 'log_status_show', width: 110, title: '<?php echo Plug_Lang('状态'); ?>'},{field: 'log_date', width: 150, title: '<?php echo Plug_Lang('时间'); ?>'},{field: 'log_remark', minWidth: 180, title: '<?php echo Plug_Lang('备注'); ?>'}]],page: true,done: function(res) {var total=0;if (res && res.extra && typeof res.extra.total_amount !=='undefined') {total=res.extra.total_amount;}jq('#total_amount_show').text('<?php echo Plug_Lang('分佣合计'); ?>: ' + total);}});jq('#searchBtn').on('click', function() {table.reload('moneylog-table', {where: {source: jq('#source').val(),soso_id: jq('#soso_id').val(),DESC: jq('#DESC').val(),soso: jq('#soso').val()},page: { curr: 1 }});});});</script>
</body>
</html>
