...
<body data="">
  ...
  <script>
    layui.config({
      ...
      index: 'lib/index'
    }).use(['index', 'set', 'form', 'laydate'], function() {
      ...
      var toggleBeizhuByMode = function(mode) {
        if (mode === 'stock') {
          jq('#beizhu-item').hide();
          jq('#beizhu').val('');
        } else {
      ...
      form.on('radio(make_mode)', function(data) {
        toggleBeizhuByMode(data.value);
      });
      toggleBeizhuByMode(jq('input[name="make_mode"]:checked').val());

      form.on('submit(set_website_2)', function(obj) {
        layer.load(1);
        ...
        jq.ajax({
          ...
          success: function(ret) {
            layer.closeAll();
            if (ret.code == 8) {
              layer.confirm(ret.msg, { btn: ['确定'] }, function() {
                window.location.href = ret.url;
              })
            } else if (ret.code == 9) {
              layer.confirm(ret.msg, { btn: ['确定', '停留当前页面'] }, function() {
                window.location.href = ret.url;
              })
            } else {
              layer.alert(ret.msg);
