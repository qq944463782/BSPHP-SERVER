...
<script>layui.config({base: '<?php echo Plug_Get_Url_Statics() ?>style/'}).extend({index: 'lib/index'}).use(['index', 'set', 'form', 'laydate'], function() {var laydate=layui.laydate;var form=layui.form;var jq=layui.jquery;laydate.render({elem: '#date',type: 'datetime'});var toggleBeizhuByMode=function(mode) {if (mode==='stock') {jq('#beizhu-item').hide();jq('#beizhu').val('');} else {jq('#beizhu-item').show();}};form.on('radio(make_mode)', function(data) {toggleBeizhuByMode(data.value);});toggleBeizhuByMode(jq('input[name="make_mode"]:checked').val());form.on('submit(set_website_2)', function(obj) {layer.load(1);var formData=jq('#bsphppost').serialize();jq.ajax({type: 'post',url: '',data: formData,success: function(ret) {layer.closeAll();if (ret.code==8) {layer.confirm(ret.msg, { btn: ['确定'] }, function() {window.location.href=ret.url;})} else if (ret.code==9) {layer.confirm(ret.msg, { btn: ['确定', '停留当前页面'] }, function() {window.location.href=ret.url;})} else {layer.alert(ret.msg);}}});return false;});});</script>
</body>
</html>
