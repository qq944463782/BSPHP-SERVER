        ...
        <div class="layui-card">
          ...
          <div class="layui-card-body">
            ...
            <script type="text/html" id="test-table-toolbar-toolbarDemo">
              <div class="layui-btn-container agent-toolbar">
                ...
                <button class="layui-btn layui-btn-sm layui-btn-normal" lay-event="act_3"><?php echo Plug_Lang('代理结构预览'); ?></button>
              </div>
            </script>
            <script type="text/html" id="test-table-toolbar-barDemo">
              <a class="layui-btn layui-btn-xs layui-btn-normal" lay-event="set"><?php echo Plug_Lang('设置'); ?></a>
              <a class="layui-btn layui-btn-xs layui-btn-normal" lay-event="order"><?php echo Plug_Lang('转账'); ?></a>
              <a class="layui-btn layui-btn-xs layui-btn-normal" lay-event="kuka_transaction_add"><?php echo Plug_Lang('库卡转出'); ?></a>
              <a class="layui-btn layui-btn-xs layui-btn-normal" lay-event="kuka_transaction_reduce"><?php echo Plug_Lang('库卡收回'); ?></a>
            </script>
          </div>
        </div>
      </div>
...
layui.config({ base: '<?php echo Plug_Get_Url_Statics() ?>style/' }).extend({ index: 'lib/index' }).use(['jquery', 'index', 'table', 'layer'], function() {
  ...
  table.render({
    ...
    cellMinWidth: 110, height: 'full-250',
    cols: [[
      {type: 'checkbox', fixed: 'left'},
      {field: 'uid', width: 80, title: '<?php echo Plug_Lang('编号'); ?>', sort: true},
      {field: 'user', minWidth: 150, title: '<?php echo Plug_Lang('用户名'); ?>', sort: true},
      {field: 'test', width: 90, title: '<?php echo Plug_Lang('状态'); ?>', sort: true},
      {field: 'LoGinNum', width: 95, title: '<?php echo Plug_Lang('登录次数'); ?>'},
      {field: 'rmb', width: 100, title: '<?php echo Plug_Lang('余额'); ?>'},
      {field: 'Zhe', width: 90, title: '<?php echo Plug_Lang('折扣'); ?>'},
      {field: 're_date', minWidth: 160, title: '<?php echo Plug_Lang('注册时间'); ?>'},
      {field: 'zhhd', minWidth: 160, title: '<?php echo Plug_Lang('最后活动'); ?>'},
      {field: 'xianka', width: 100, title: '<?php echo Plug_Lang('现卡总数'); ?>'},
      {field: 'kuka', width: 100, title: '<?php echo Plug_Lang('库卡总数'); ?>'},
      {field: 'agent_beizhu', minWidth: 180, title: '<?php echo Plug_Lang('备注(编辑)'); ?>', edit: 'text'},
      {fixed: 'right', title: '<?php echo Plug_Lang('操作'); ?>', toolbar: '#test-table-toolbar-barDemo', width: 280}
    ]],
    page: true
  });
});
