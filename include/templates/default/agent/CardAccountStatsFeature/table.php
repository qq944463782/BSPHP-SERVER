...
<script>layui.config({base: '<?php echo Plug_Get_Url_Statics() ?>style/'}).extend({index: 'lib/index'}).use(['jquery', 'index', 'set', 'laydate', 'layer'], function() {var laydate=layui.laydate,jq=layui.jquery,admin=layui.admin;laydate.render({elem: '#date1',type: 'date'});laydate.render({elem: '#date2',type: 'date'});jq('#soso').on('click', function() {layer.open({type: 2,title: '<?php echo Plug_Lang('选择代理'); ?>',area: ['700px', '450px'],fixed: false,maxmin: true,content: 'index.php?m=agent&c=CardAccountStatsFeature&a=agent_list&val=' + jq('#soso').val() + "&id=soso"});})});function openka(daihao, kalei, sdate, name) {var date1=document.getElementById("date1").value;var date2=document.getElementById("date2").value;var agenttype=document.getElementById("agenttype").value;if (sdate=='all') {var zhuangtai=-1;var on=-1;var date_type=-1;if (date1 !='' || date2 !='') {date_type=1;}}if (sdate=='in') {var zhuangtai=1;var on=1;var date_type=0;}if (sdate=='ok') {var zhuangtai=1;var on=2;var date_type=1;}if (sdate=='look') {var zhuangtai=2;var on=-1;var date_type=1;}var soso=name;if (agenttype==0) {var soso_id=5;}if (agenttype==1) {var soso_id=6;}var index=layer.open({type: 2,title: '<?php echo Plug_Lang('卡列表'); ?>',area: ['700px', '450px'],fixed: false,maxmin: true,content: "index.php?m=agent&c=sp&a=table&daihao=" + daihao + "&agenttype=" + agenttype + "&soso_ok=1&soso_id=" + soso_id + "&soso=" + soso + "&DESC=0&zhuangtai=" + zhuangtai + "&on=" + on + "&date_type=" + date_type + "&date1=" + date1 + "&date2=" + date2 + "&kalei=" + kalei});layer.full(index);}</script>
<script language="javascript">select_set_text('agenttype', <?php echo Plug_Set_Get('agenttype'); ?>);select_set_text('daihao', <?php echo Plug_Set_Get('daihao'); ?>);select_set_text('soso_id', <?php echo Plug_Set_Get('soso_id'); ?>);</script>
</body>
</html>