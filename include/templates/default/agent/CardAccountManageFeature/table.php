   ...
   var Bsphp_G_E = '<?php echo Plug_Lang('条'); ?>';
    layui.config({
      ...
      table.render({
        ...
        toolbar: '#test-table-toolbar-toolbarDemo',
        title: '<?php echo Plug_Lang('卡户数据表'); ?>',
        height: 'full-290',
        cols: [
          [

            {
              ...
              field: 'vip_unix',
              width: 160,
              title: '<?php echo Plug_Lang('到期时间'); ?>',
            }, {
              field: 'vip_status',
              width: 100,
              title: '<?php echo Plug_Lang('状态'); ?>',
            }, {
              field: 'login_time',
              width: 160,
              title: '<?php echo Plug_Lang('登录时间'); ?>',
            }, {
        ...
        where: {
          ...
          expire_status: document.getElementById("expire_status").value,
      ...
      table.on('toolbar(test-table-toolbar)', function(obj) {
        ...
        switch (obj.event) {
          ...
          case 'batch_unlock':
            ...
            break;
          case 'batch_delete':
            layer.confirm('<?php echo Plug_Lang('确认批量删除卡密及关联用户？一次最多50条'); ?>', function(index) {
              layer.close(index);
      ...
      table.on('tool(test-table-toolbar)', function(obj) {
        ...
        if (obj.event === 'lock_1') {
          ...
          layer.confirm('<?php echo Plug_Lang('确认删除卡密及关联用户？'); ?>', function(index) {
            layer.close(index);
            jq.ajax({
              type: "post",
              url: '',
    ...
    select_set_text('expire_status', '<?php echo Plug_Set_Get('expire_status'); ?>');
