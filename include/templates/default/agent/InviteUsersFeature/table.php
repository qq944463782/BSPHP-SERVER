...
<style>.invite-link-box { margin-bottom: 14px; border: 1px solid #f0f0f0; border-radius: 8px; }.invite-link-title { font-weight: 600; margin-bottom: 6px; }.invite-link-desc { color: #666; margin-bottom: 10px; }.invite-link-url { word-break: break-all; color: #1e9fff; margin-bottom: 10px; }</style>
</head>
<body>
...
<script>layui.config({ base: '<?php echo Plug_Get_Url_Statics() ?>style/' }).extend({ index: 'lib/index' }).use(['jquery', 'index', 'layer'], function() {var layer=layui.layer;window.copyText=function(id) {var el=document.getElementById(id);var txt=el ? (el.textContent || el.innerText) : '';if (!txt) return;if (navigator.clipboard && navigator.clipboard.writeText) {navigator.clipboard.writeText(txt).then(function () {layer.msg('<?php echo Plug_Lang('复制成功'); ?>');}, function () {fallbackCopy(txt);});} else {fallbackCopy(txt);}};function fallbackCopy(txt) {var input=document.createElement('textarea');input.value=txt;document.body.appendChild(input);input.select();try { document.execCommand('copy'); } catch (e) {}document.body.removeChild(input);layer.msg('<?php echo Plug_Lang('复制成功'); ?>');}});</script>
</body>
</html>
