...
class appweb
{
    ...
    /**
     * @登录
     * http://127.0.0.5/index.php?m=applib&c=appweb&a=login&BSphpSeSsL=[验证串:可空]&user=[用户账户]
     */
    function call_login()
    {
        $log_name = '';
    ...
    /**
     * @登录模式
     * http://127.0.0.5/index.php?m=applib&c=appweb&a=links_login&daihao=[软件代号:可空]&BSphpSeSsL=[验证串:可空]&user=[用户账户]&pwd=[用户密码]&login=YES
     * 
     */
    ...
    /**
     * @登录模式
     * http://127.0.0.5/index.php?m=applib&c=appweb&a=links_car&daihao=[软件代号]&BSphpSeSsL=[验证串:可空]&car_id=[卡用户账户]&car_pwd=[用户密码:可空]&login=YES
     * 
     */
    ...
    function call_buyvip()
    {
        ...
        if ($uid == '') {
            echo Plug_Lang('[&uid=]充值对象不能为空,请输入充值开通账户/卡模式充值卡号/机器码');
            exit;
        }
        ...
        if (!file_exists($openfileconfig))
            exit(Plug_alerts(Plug_Lang('Error:要引入支付文件不存在'), Plug_Lang('需要引入php脚本文件不存在。<BR/>文件路径:') . $openfileconfig));
