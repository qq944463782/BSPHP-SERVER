...
/*
 * Feature: 批量删除
 * Menu ID: batch_delete
 * 说明: 批量删除充值卡入口页，统一执行菜单权限校验。
 */
defined('BSPHP_SET') or die('Not,This File Not Can in Ie Open');
if (BSPHP_SET != 'AGENT')
    die('Not,This File Not Can in Ie Modules');

class BatchDeleteFeature
{
    ...
    function __construct()
    {
        ...
        //判断用户是否有权限使用批量删除功能
        Plug_Agent_Assert_Menu('batch_delete', $this->Grade, false);
    }
    ...
    function call_table()
    {
        ...
        if ($baocun) {
                ...
                if (Plug_Get_Configs_Value('agents', "car_delete_{$this->Grade}") == 1) {
                    ...
                    if ($array['car_IsLock'] == 1) {
                        ...
                        if ($car_pur_date > $car_on_time) {
                            $ka_txt .= $v . Plug_Lang(" >>>激活时间超") . "{$car_on_time}" . Plug_Lang("秒,无法删除") . "\n";
                            $ka_txt2 .= $v . "\n";
                            continue;
                        }
                    }
                    ...
                    if (Plug_Get_Configs_Value('agents', "car_on_{$this->Grade}") == 1) {
                        if ($array['car_cong_user'] == 'cardid') {
                            $sql = "DELETE FROM `bs_php_pattern_login`  WHERE `L_daihao`='{$array['car_DaiHao']}' AND   `L_User_uid` ='{$array['car_name']}';";
                            Plug_Query($sql);
                        } else {
                            $sql = "DELETE FROM `bs_php_pattern_login`  WHERE `L_daihao`='{$array['car_DaiHao']}' AND `L_User_uid` ='{$array['car_chong_uid']}' AND `L_ic_pwd` ='{$array['car_chong_uid']}';";
                            Plug_Query($sql);
                        }
                    }
                    $ka_txt .= $v . Plug_Lang(" >>>删除已经执行") . "\n";
                    $ka_txt1 .= $v . "\n";
                    continue;
                } else {
                    $ka_txt .= $v . Plug_Lang(" >>>没有权限删除\n");
                }
            }
            ...
            $jieguo = "" .
                ...
                "$ka_txt" .
                Plug_Lang("-------------" . Plug_Lang("删除已经执行") . "-------------") . PHP_EOL .
                "$ka_txt1" .
                Plug_Lang("-------------" . Plug_Lang("激活时间超") . "") . "{$car_on_time}" . Plug_Lang("秒,无法删除-------------") . PHP_EOL .
                "$ka_txt2" .
