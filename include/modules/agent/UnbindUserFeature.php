...
class UnbindUserFeature
{
    ...
    function call_table()
    {
        ...
        if ($appenconfig) {
            ...
            if (Plug_Bind_Key_Count($pattern_login) <= 0 && $key == '') {
                Plug_Print_Json(array('code' => 1, 'msg' => Plug_Lang('当前没有绑定,无需解绑.')));
            }

            if ($key === '') {
                $tmp = Plug_Bind_Key_Clear($pattern_login);
            } else {
                Plug_Bind_Key_Clear($pattern_login);
                $pattern_login['L_key_info'] = '';
                $tmp = Plug_Bind_Key_Add($pattern_login, $key);
            }
            if ($tmp) {
