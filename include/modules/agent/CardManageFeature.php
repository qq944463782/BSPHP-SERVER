...
class CardManageFeature
{
    ...
    function __construct()
    {
        ...
        if (Plug_Get_Session_Value('USER_UID_IS') == 0) {
            Plug_ShowMsg(Plug_Lang("代理中心未授权,请授权后再使用。"), '');
    ...
    function call_table_json()
    {
        ...
        $sql = "SELECT `app_daihao`,`app_name`,`app_MoShi` FROM `bs_php_appinfo`";
        ...
        if ($dbs_array_value) {
            while ($array_value = Plug_Pdo_Fetch_Assoc($dbs_array_value)) {
                ...
                $app_moshi[$array_value['app_daihao']] = $array_value['app_MoShi'];
            }
        }
        ...
        while ($array_value = Plug_Pdo_Fetch_Assoc($db_array_value)) {
            ...
            $while_array_list['app_date'] = Plug_Card_Type_Date_Label(
                $array_value['car_TianShu'],
                $array_value['car_type'],
                isset($app_moshi[$array_value['car_DaiHao']]) ? $app_moshi[$array_value['car_DaiHao']] : ''
            );
