...
$GLOBALS['WEBAPI_META'] = array(
    'id'    => 'salecard_salecard',
    'name'  => '销售卡3-WEB页面销售导入的卡(制卡导入标识为用销售卡)',
    'path'  => '/index.php',
    'method'=> 'GET',
    'fixed' => array('m' => 'webapi', 'c' => 'salecard_salecard', 'a' => 'index'),
    'params'=> array(
...
/**
 * 销售现卡 - 销售制卡导入时标识为「用销售现卡」的卡，支付成功后从库存分配并标记 car_buy_by=订单号
 */
class salecard_salecard
{
    ...
    /**
     * 全部软件列表页 - 销售卡3 现卡销售入口聚合
     * 访问: index.php?m=webapi&c=salecard_salecard&a=list
     */
    function call_list()
    {
    ...
    /**
     * 现卡库存数（car_sale_flag=1 且同软件、同卡类）
     */
    function _get_stock_count($daihao, $lei_id)
    {
        $daihao = (int)$daihao;
        $lei_id = (int)$lei_id;
        $sql = "SELECT COUNT(*) AS cnt FROM `bs_php_cardseries` WHERE `car_DaiHao`='{$daihao}' AND `car_Lei`='{$lei_id}' AND `car_sale_flag`='1' LIMIT 1";
        $row = Plug_Query_Assoc($sql);
        return isset($row['cnt']) ? (int)$row['cnt'] : 0;
    }

    /**
     * 支付入口：创建订单并跳转支付（仅当现卡库存足够）
     */
    function call_do_pay()
    {
        ...
        $stock = $this->_get_stock_count($daihao, (int)$carid);
        if ($stock < $shuliang) {
            plug_print_json(array('code' => -1, 'msg' => Plug_Lang('现卡库存不足') . '（当前库存 ' . $stock . ' 张）'));
            return;
        }
        $sql = "SELECT * FROM `bs_php_appinfo` WHERE `app_daihao`='{$daihao}' LIMIT 1";
        $appinfo = Plug_Query_Assoc($sql);
        ...
        $pay_beizhu = Plug_Lang("{$app_name}") . Plug_Lang(" - 现卡 ") . Plug_Lang("{$arr['lei_name']}");
        if ($shuliang > 1) {
        ...
        $ail_id = 'SALEIMP' . date('YmdHis', PLUG_UNIX()) . rand(1000, 99999);
        $ka_kate = PLUG_DATE();
    ...
    function _get_cards_by_order($order)
    {
        ...
        $sql = "SELECT c.`car_name`,c.`car_pwd`,c.`car_reDATE`,c.`car_TianShu`,k.`lei_name` 
                ...
                WHERE c.`car_buy_by`='{$order}' ORDER BY c.`car_id` ASC";
        $list = array();
        $res = Plug_Query($sql);
        if ($res) {
            while ($row = Plug_Pdo_Fetch_Assoc($res)) {
    ...
    {
        $order = preg_replace('/[^a-zA-Z0-9]/', '', $order);
        if (strlen($order) < 10 || substr($order, 0, 7) !== 'SALEIMP') {
            echo Plug_Lang('订单号格式错误，请使用SALEIMP开头订单号');
            exit;
        }
        ...
        if (empty($cards)) {
            ...
            if ($pl) {
                echo Plug_Lang('订单已支付完成，卡密分配中或已发放。若未收到请联系客服，订单号：') . htmlspecialchars($order);
            } else {
