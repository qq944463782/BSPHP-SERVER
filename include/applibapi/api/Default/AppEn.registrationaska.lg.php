...
$tmp = Plug_Query_Assoc($sql);
if (!$tmp) {
    Plug_Echo_Info(Plug_Lang("激活码无效"), -1); //当前绑定特征已经有人绑定了 
...
if ($extension > 0) {
    ...
    if (!$uid_extension)
        Plug_Echo_Info(Plug_Lang('推荐人不错误,没有请不要填写！'), -2);
}
/**
 * 用户注册
 */
...
if ($log == 1005) {
    ...
    if ($gift_value == 0) {
        ...
        $s = $gift_value / 3600;
        Plug_Echo_Info(Plug_Lang('注册成功,恭喜你获得了') . round($s, 1) . Plug_Lang('小时的使用权'), 1005);
    }
}
