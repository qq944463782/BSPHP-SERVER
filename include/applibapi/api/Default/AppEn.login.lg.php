...
if ($log == 1011) { #登录成功
    ...
    #日志记录：账号、绑定机器码、多开机器码
    Plug_Add_AppenLog('user_login_log', Plug_Lang('账号模式登录 账号:') . $user . Plug_Lang(' 机器码:') . $key . Plug_Lang(' 多开机器码:') . $maxoror, $user);
    ...
    if ($arr['L_vip_unix'] > PLUG_UNIX()) {
        ...
        if (Plug_App_DaTa('app_set') == 1) {
            ...
            if ($bind_ok !== 1) {
                //注销登录
                Plug_Set_Session_Value('USER_UID', ''); //登录UID
                Plug_Set_Session_Value('USER_YSE', ''); //登录MD7加密
                Plug_Set_Session_Value('USER_DATE', ''); //上一次登录时间
                Plug_Set_Session_Value('USER_IP', ''); //上一次登录IP
