...
/*
<api>
  <name>get_app_user_extra.lg</name>
...
/**
 * get_app_user_extra.lg
 * 获取当前登录用户在当前软件的拓展字段（JSON字符串）
 * 返回示例：{"姓名":"张三","学校":"xx中学"}
 */
...
$sql = "SELECT `L_user_extra` FROM `bs_php_pattern_login` WHERE `L_User_uid`='{$uid}' AND `L_daihao`='{$daihao}' ORDER BY `L_id` DESC LIMIT 1;";
$row = Plug_Query_Assoc($sql);
$user_extra = ($row && isset($row['L_user_extra'])) ? (string)$row['L_user_extra'] : '';
if ($user_extra === '') $user_extra = '{}';
