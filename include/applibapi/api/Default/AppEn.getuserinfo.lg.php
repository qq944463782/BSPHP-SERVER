...
<api>
  ...
  <params>
    <param name="info" required="false" type="string" desc="需要信息歌手字段"></param>
  </params>
</api>
*/
...
$daihao = PLUG_DAIHAO();
$info = Plug_Set_Data('info');    #需要信息歌手字段
if ($info == NULL) Plug_Echo_Info('请输入要获取数据类型如:激活时间=UserReDate,激活时Ip=UserReIp,用户状态=UserIsLock,登录时间=UserLogInDate', -1);
...
if ($log == 1047) {
   ...
   //计算剩余倒计时用全球化显示
   $endTime = new DateTime(date('Y-m-d H:i:s', $arr['L_vip_unix']));
   ...
   $info = str_replace('UserVipDate', date('Y-m-d H:i:s', $arr['L_vip_unix']), $info);
