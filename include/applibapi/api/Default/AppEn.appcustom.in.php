...
/*
<api>
  <name>appcustom.in</name>
  <title>取自定义配置值（后台应用管理-自定义配置中配置的 field_key）</title>
    <intro>接口参数说明</intro>
  ...
  <params>
    <param name="info" required="false" type="string" desc="要获取的 field_key 列表，用 | 分隔"></param>
    <param name="get_type" required="false" type="string" desc="参数说明"></param>
    <param name="user" required="false" type="string" desc="用户名"></param>
    <param name="pwd" required="false" type="string" desc="密码"></param>
    <param name="icid" required="false" type="string" desc="参数说明"></param>
    <param name="icpwd" required="false" type="string" desc="参数说明"></param>
  </params>
</api>
*/
...
if ($app_moshi == 'CardTerm' || $app_moshi == 'CardPoint') {
    ...
    if ($car_id != '' || $car_pwd != '') {
        ...
        if ($log == 1080) {
            ...
            if ($log != 1080) {
                Plug_Echo_Info('未登录,登录后才能获取配置值', 5033);
            }
        }
    } else {
...
foreach ($keys as $key) {
    ...
    if ($row && isset($row['field_val'])) {
        $login_req = $row['login_required']; //登录后可取 0=无需登录 1=需登录 2=需要登录且没过期才可取
        ...
        if ($login_req == 1) {
            ...
            if (!$is_login_ok) {
                $val = '';  # 需登录但未登录，返回空
                Plug_Echo_Info($key_safe . '未登录,登录后才能获取配置值', 5033);
            }
            //正常返回配置值    
            $val = $row['field_val'];
        } elseif ($login_req == 2) {
            ...
            //登录卡模式扣点模式判断 判断是否到期的配置 扣点不需要时间判断
