...
<api>
  ...
  <params>
    <param name="user" required="false" type="string" desc="账号密码方式验证登录"></param>
    <param name="pwd" required="false" type="string" desc="账号密码方式验证登录"></param>
  </params>
</api>
*/
...
if ($log == 1047 or $log == 1011) {
    ...
    //判断是否已经绑定过了
    if (Plug_Bind_Key_Count($arr) <= 0)
        Plug_Echo_Info('已经解除绑定了', 200);
    ...
    if ($arr['L_vip_unix'] > PLUG_UNIX()) {
        ...
        //解除全部绑定（扣时一次）
        $tmp = Plug_Bind_Key_Clear($arr);
        if ($tmp) {
            Plug_Query("UPDATE`bs_php_pattern_login`SET`L_vip_unix`='$date' WHERE`L_id` ='{$arr['L_id']}'");
