...
/*
<api>
  <name>resetpwd_email.lg</name>
  <title>邮箱验证码找回密码</title>
    <intro>接口参数说明</intro>
...
if ($email == '' || $email_code == '' || $pwd == '' || $pwdb == '') {
    Plug_Echo_Info(Plug_Lang('请填写邮箱、验证码和新密码.'), -1);
    exit;
}
if ($pwd !== $pwdb) {
...
if (stristr(Plug_App_DaTa('app_coode'), 'resetpwd_email.lg')) {
...
if (!Plug_Otp_Verify_Email('reset', $email, $email_code)) {
    Plug_Echo_Info(Plug_Lang('验证码错误或已过期.'), -1);
    exit;
}

$email_safe = addslashes($email);
$row = Plug_Query_Assoc("SELECT `user_uid` FROM `bs_php_user` WHERE `user_email`='{$email_safe}' LIMIT 1");
if (!$row) {
