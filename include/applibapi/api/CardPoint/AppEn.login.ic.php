...
if ($log == 1081) {
    ...
    //-----------------------------------------
    //记录登录日志
    Plug_Add_AppenLog('user_login_log', Plug_Lang('卡扣点模式登录 卡号:') . $icid . Plug_Lang(' 机器码:') . $key . Plug_Lang(' 多开机器码:') . $maxoror, $icid);
    //记录登录时间用做扣点
    ...
    $login_info = NULL;
    if (Plug_Bind_Key_Check($carinfo, $key) || $ic_key != '') { if (Plug_Bind_Key_Check($carinfo, $key)) $login_info = Plug_App_data('app_logininfo'); }
