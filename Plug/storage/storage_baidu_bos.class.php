...
$this->access_key_id=trim($cfg['baidu_access_key_id'] ?? '');
$this->secret_access_key=trim($cfg['baidu_secret_access_key'] ?? ($cfg['baidu_access_key_secret'] ?? ''));
$this->bucket=trim($cfg['baidu_bucket'] ?? '');
$ep=trim($cfg['baidu_endpoint'] ?? 'bj.bcebos.com');
$this->endpoint=(strpos($ep, 'http')===0) ? rtrim($ep, '/') : ('https://' . $this->bucket . '.' . $ep);
$this->custom_domain=trim($cfg['baidu_custom_domain'] ?? '');
}
...
$url=$this->endpoint . '/' . $object_key;
$date=gmdate('D, d M Y H:i:s \G\M\T');
$content_type=$this->getContentType($object_key);
$content_length=strlen($file_content);
$string_to_sign="PUT\n\n" . $content_type . "\n" . $date . "\n/" . $this->bucket . '/' . $object_key;
$signature=base64_encode(hash_hmac('sha1', $string_to_sign, $this->secret_access_key, true));
$auth='bce-auth-v1/' . $this->access_key_id . '/' . date('Y-m-d') . '/1800/host/' . $signature;
...
return rtrim($this->endpoint, '/') . '/' . $object_key;
}
}
